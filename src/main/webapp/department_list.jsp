    ...
    layui.use(['layer', 'table','form','jquery'], function(){
        ...
        table.render({
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'depId', title: 'ID', sort: true}
                ,{field: 'depNo', title: '部门编码'}
                ,{field: 'depName', title: '部门名称'}
                ,{field: 'depLeader', title: '部门负责人'}
                ,{field: 'depDuty', title: '部门职责'}
                ,{field: 'depHigher', title: '上级部门'}
        ...
        table.on('toolbar(test)', function(obj){
            ...
            switch(obj.event){
                case 'add':
                    xadmin.open('添加用户','/dep/type?type=add',600,400)
                    break;
                case 'update':
                    if(data.length === 0){
                        layer.msg('请选择一行');
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                ...
                xadmin.open('编辑','/dep/type?type=edit&depId='+data.depId,600,400)
            }
        });
        var $ = layui.$, active = {
            //layui 搜索模块
