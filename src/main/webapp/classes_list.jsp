    ...
    layui.use(['layer', 'table','form','jquery'], function(){
        ...
        table.render({
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'clId', title: 'ID', sort: true}
                ,{field: 'clNo', title: '班次编码'}
                ,{field: 'clName', title: '班次名称'}
                ,{field: 'clMorning',title: '早上上班时间'}
                ,{field: 'clEvening', title: '下午下班时间'}
        ...
        table.on('toolbar(test)', function(obj){
            ...
            switch(obj.event){
                case 'add':
                    xadmin.open('添加','/cl/type?type=add',800,500)
                    break;
                case 'update':
                    if(data.length === 0){
                        layer.msg('请选择一行');
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                ...
                xadmin.open('编辑','/cl/type?type=edit&clId='+data.clId,800,500)
            }
        });
        var $ = layui.$, active = {
            //layui 搜索模块
