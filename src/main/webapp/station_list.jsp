    ...
    layui.use(['layer', 'table','form','jquery'], function(){
        ...
        table.render({
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'staId', title: 'ID', sort: true}
                ,{field: 'staNo', title: '岗位编码'}
                ,{field: 'staName', title: '岗位名称'}
                ,{field: 'depName', templet: '<div>{{createrFormat(d.department.depName)}}</div>',title: '所在部门'}
                ,{field: 'staSuperior', title: '直接上级'}
        ...
        table.on('toolbar(test)', function(obj){
            ...
            switch(obj.event){
                case 'add':
                    xadmin.open('添加','/sta/type?type=add',600,400)
                    break;
                case 'update':
                    if(data.length === 0){
                        layer.msg('请选择一行');
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                ...
                xadmin.open('编辑','/sta/type?type=edit&staId='+data.staId,600,400)
            }
        });
        var $ = layui.$, active = {
            //layui 搜索模块
